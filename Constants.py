...
# with open(ROOT_DIR / 'meta_info/GQA_hypernym.json') as f:
#     hypernym = json.load(f)

with open(ROOT_DIR / 'meta_info/objects.json') as f:
    OBJECTS_INV = json.load(f)
    OBJECTS = {k: i for i, k in enumerate(OBJECTS_INV)}

with open(ROOT_DIR / 'meta_info/predicates.json') as f:
    RELATIONS_INV = json.load(f)
    RELATIONS = {k: i for i, k in enumerate(RELATIONS_INV)}

with open(ROOT_DIR / 'meta_info/attributes.json') as f:
    ATTRIBUTES_INV = json.load(f)
    ATTRIBUTES = {k: i for i, k in enumerate(ATTRIBUTES_INV)}

with open(ROOT_DIR / 'meta_info/obj2attribute.json') as f:
    mapping = json.load(f)
